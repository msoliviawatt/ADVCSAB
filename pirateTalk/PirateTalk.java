...
//			if the word meets neither of the above cases, return the same word
   ...
   {
      word = clean(word);
      if(!(englishToPirate.get(word) == null)) {
...
//			another element might have the key "there" and the value ["thar"] 
   ...
   public Map<String, ArrayList<String>> flip()
   {
      ...
      for(String word : englishToPirate.keySet()) {
         String translation = englishToPirate.get(word);
         if(!(ans.get(translation) == null)) {
            ans.get(translation).add(word);
         }
         else {
            ArrayList<String> helper = new ArrayList<String>();
            helper.add(word);
            ans.put(translation, helper);
         }
      }
