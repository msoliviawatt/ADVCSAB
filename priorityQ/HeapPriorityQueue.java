...
public class HeapPriorityQueue implements PriorityQueue
{
   ...
   private void reheapUp()
   {
      ...
      while(currentIndex > 1) {
         if((higherPriority(items[currentIndex], items[currentIndex / 2]))) {
   ...
   {
      int currentIndex = 1;
      while (currentIndex * 2 <= numItems) {
         int nextIndex = currentIndex * 2;
         if (currentIndex * 2 + 1 <= numItems && higherPriority(items[currentIndex * 2 + 1], items[currentIndex * 2])) { 
               nextIndex += 1;
         }
         if (higherPriority(items[nextIndex], items[currentIndex])) {
            swap(items, currentIndex, nextIndex);
